...
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_SECURE` | no | `SMTP_PORT` defaults to `587`, `SMTP_SECURE` to `false` | Second email transport (via [Nodemailer](https://nodemailer.com)), used only when `RESEND_API_KEY` is unset. Lets you reuse an existing mailbox — e.g. Gmail + an [App Password](https://myaccount.google.com/apppasswords) at `smtp.gmail.com:587` with `SMTP_SECURE=false` (STARTTLS). All four of `SMTP_HOST`/`SMTP_USER`/`SMTP_PASS` (port has a default) must be set together — a partial config is treated as "not configured", not a startup error. |
| `EMAIL_FROM` | no | `OpenOTA <onboarding@resend.dev>` | `From` address for verification/reset emails (used by whichever transport — Resend or SMTP — is active) |
