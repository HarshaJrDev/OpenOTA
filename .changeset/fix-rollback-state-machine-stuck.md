---
"@openota/native-android": patch
---

Fix two related bugs in the runtime state machine's `ROLLBACK` handling, both found via real
...
1. **A device could get permanently stuck after any rollback.** `stateHolder` is constructed
   directly from the *persisted* root manifest state on every `BundleManager` construction — a
